...
Tensor& maximum_out(
  ...
  if(!fall_back)
  {
    ...
    if(broadcast == 1)
    {
       ...
       int max = (a.dim() > b.dim()) ? a.dim() : b.dim();
       max = (max > out.dim()) ? max : out.dim();
         
       int off_o = max - out.dim();
       int off_a = max - a.dim();
       int off_b = max - b.dim();
       ...
       xa_nn_elm_maximum_broadcast_4D_f32xf32_f32(out_data, out_shape, a_data, inp1_shape, b_data, inp2_shape);
    }
    else
    {
