  ...
  if (optimized) {
    ...
    if (input.scalar_type() == ScalarType::Char) {
      ...
      for (int i = 0; i < out_channels; i++) {
        out_multiplier32[i] =
            bias_scale.const_data_ptr<float>()[0] * out_scale * 2147483648;
        out_shift32[i] = 0;
      }
      ...
      WORD8* ptr1 = (WORD8*)malloc(
          ((input.size(0) * input_channels * input_height * input_width) + 8) *
          sizeof(WORD8));
      WORD8* ptr2 = (WORD8*)malloc(
      ...
      const UWORD8* __restrict__ p_kernel1 =
          (const UWORD8* __restrict__)pkernel;

      for (int _n = 0; _n < batches; _n++) {
        const UWORD8* __restrict__ in_batch =
            p_inp1 + _n * input_channels * input_height * input_width;
        UWORD8* __restrict__ out_batch =
