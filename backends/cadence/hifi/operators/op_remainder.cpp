  ...
  static void run(const Tensor& a, const Tensor& b, Tensor& out) {
    torch::executor::apply_binary_elementwise_fn<CTYPE_A, CTYPE_B, CTYPE_OUT>(
        ...
        [](const CTYPE_A val_a, const CTYPE_B val_b) {
          ...
          CTYPE_IN value = torch::executor::native::utils::remainder_override(
              a_casted, b_casted);
...
Tensor& remainder_Tensor_out(
  ...
  constexpr auto name = "remainder.Tensor_out";
  ...
  if (optimized) {
    if (broadcast) {
      ...
      xa_nn_elm_remainder_f32xf32_f32(p_out, p_inp1, p_inp2, num_elm);
    }
    return out;
  }
  ...
  ET_SWITCH_REAL_TYPES_AND(Bool, a_type, ctx, name, CTYPE_A, [&]() {
    ET_SWITCH_REAL_TYPES_AND(Bool, b_type, ctx, name, CTYPE_B, [&]() {
      ...
      ET_SWITCH_REAL_TYPES(out_type, ctx, name, CTYPE_OUT, [&]() {
        RemainderInner<
...
Tensor& remainder_Scalar_out(
  ...
  constexpr auto name = "remainder.Scalar_out";
        ...
        ET_SWITCH_REAL_TYPES(out_type, ctx, name, CTYPE_OUT, [&]() {
          torch::executor::apply_unary_map_fn(
              [val_b](const CTYPE_A val_a) {
                ...
                CTYPE_IN b_casted = static_cast<CTYPE_IN>(val_b);
                CTYPE_IN value =
                    torch::executor::native::utils::remainder_override(
                        a_casted, b_casted);
